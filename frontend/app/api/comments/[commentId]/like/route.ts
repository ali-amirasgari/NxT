...
export async function POST(request: NextRequest, { params }: { params: Promise<{ commentId: string }> }) {
  ...
  return proxyToBackend(request, { method: "POST", upstreamPath: API_ROUTES.social.upstream.comments.like(commentId) });
}

export async function DELETE(request: NextRequest, { params }: { params: Promise<{ commentId: string }> }) {
  const { commentId } = await params;
  return proxyToBackend(request, { method: "DELETE", upstreamPath: API_ROUTES.social.upstream.comments.like(commentId) });
}
